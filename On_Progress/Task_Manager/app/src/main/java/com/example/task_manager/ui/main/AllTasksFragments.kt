...
class AllTasksFragment(private val viewModel: TaskViewModel) : Fragment() {

    private lateinit var binding: FragmentTasksBinding
    private lateinit var adapter: TaskAdapter

    companion object {
        fun newInstance(viewModel: TaskViewModel) = AllTasksFragment(viewModel)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        adapter = TaskAdapter(
            emptyList(),
            onTaskClick = { task ->
                AddEditTaskDialog(task) { updated ->
