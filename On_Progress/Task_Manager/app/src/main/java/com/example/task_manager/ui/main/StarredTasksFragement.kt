...
class StarredTasksFragment(private val viewModel: TaskViewModel) : Fragment() {

    private lateinit var binding: FragmentTasksBinding
    private lateinit var adapter: TaskAdapter

    companion object {
        fun newInstance(viewModel: TaskViewModel) = StarredTasksFragment(viewModel)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.tasks.observe(viewLifecycleOwner) { taskList ->
            val starredTasks = taskList.filter { it.isStarred }
            adapter.updateData(starredTasks)
        }
    }
}
